...
```
F('looked at') = lambda (x,y) in A * A: 1 iff (x,y) in {(x,y) : x looked at y} <= A * A
Type: (e * e) -> t 
```
...
> ```
> F('looked at')(x,y) = looked_at_f(x,y) 1 iff (x,y) in {(x,y): x looked at y} <= A * A
> ```
> Using the $\lambda$ operator to bind $x$ return another function:
> ```
> lambda x : 1 iff (x,y) in {(x,y): x looked at y} <= A * A
> ```
> Using the $\lambda$ operator to bind $y$ return another function:
> ```
> lambda y : lambda x : 1 iff (x,y) in {(x,y): x looked at y} <= A * A
> ```
> As a result, ...
> ```
> F('looked at') = lambda y : lambda x: 1 iff (x,y) in {(x,y): x looked at y} <= A * A
>                = lambda y : lambda x: looked_at_f(x,y)
> Type: e -> e -> t
> ```

> [!NOTE]
> The order in which arguments are applied is the **reverse** of the order of elements within the pairs.
> ```
> lambda y : lambda x : looked_at_f(x,y)  # lambda y lambda x ... (x, y) 
> ``` 

> [!NOTE]
> This process is known as ['Currying'](https://en.wikipedia.org/wiki/Currying) in mathematics and computer science.
...
[Next session](https://github.com/haozeli-ling/Semantic-Analysis/blob/main/09.%20Common%20nouns%20and%20adjectives.md)
