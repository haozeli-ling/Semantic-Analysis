...
```
F('looked at') = lambda (x,y) in A X A: 1 iff (x,y) in {(x,y) : x looked at y}
Type: (e X e) -> t 
```
...
> ```
> F('looked at')(x,y) = 1 iff (x,y) in {(x,y): x looked at y}
> ```
> Using the $\lambda$ operator to bind $x$ return another function:
> ```
> lambda x. 1 iff (x,y) in {(x,y): x looked at y}
> ```
> Using the $\lambda$ operator to bind $y$ return another function:
> ```
> lambda y lambda x : 1 iff (x,y) in {(x,y): x looked at y}
> ```
> As a result, ...
> ```
> F('looked at') = lambda y lambda x: 1 iff (x,y) in {(x,y): x looked at y}
> Type: e -> e -> t
> ``` 
