...
In our algorithm, we model presuppositions as definedness conditions. 

- $⟦\text{the president}⟧^M$ = the entity $x$ **defined only if** $\text{present}'(x) = 1$ in $M$

As a definedness condition, the failure of the uniqueness condition makes the whole sentence undefined. It means that 
