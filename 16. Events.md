...
Based on the previous exercise, the semantic value of Sentence 4 is a function from events to truth values, since the event variable is bound by a lambda operator rather than existentially closed. Therefore, an additional mechanism is required to derive full truth conditions in event semantics. Tense morphemes serve this function. 

```
F('-ed') = lambda f : exists e : f(e) and time_f(e) < ST
Type :: (v -> t) -> t
```

The past tense morpheme is interpreted at the clausal level and requires that the event described by the clause occur prior to the speech time (ST).

```
F('Daisy hugged Gatsby')
= F('-ed')(F('Daisy hug Gatsby'))
= [lambda f : exists e : f(e) and time_f(e) < ST] (lambda e : hug_f(e) and agent_f(e, "d") and theme_f(e, "g"))
= exists e : hug_f(e) and agent_f(e, "d") and theme_f(e, "g") and time_f(e) < ST
```

The present tense morpheme and the future tense morpheme can be defined in a similar way: 

```
F('-s') = lambda f : exists e : f(e) and time_f(e) o ST
F('will') = lambda f : exists e : f(e) and time_f(e) > ST
```
...
To resolve this problem, we 'raise' quantifiers and interpret them after the tense morphemes have been applied.

> Exercise: Complete the following semantic computation. 
>
> ```
> F('every student [1 -s [pro_1 be smart]]')
> =
> ```
