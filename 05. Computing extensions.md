...
- $⟦\text{Sue}⟧^M$ = $\text{s}$
- $⟦\text{Sue danced}⟧^M$ = 1 (suppose  $\text{s} \in$ { $x$ | $x$ danced } \subseteq A)
...
**Characteristic Function**: a function mapping an object to a truth value, such as $\lambda x.\text{danced}'(x)$. 
