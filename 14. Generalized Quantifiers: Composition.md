...
| $\wedge$ | and | conjunction of statements |

Truth condition:
```
F('every man smokes') = {x : man_f(x) = 1} <= {x : smokes_f(x) = 1}
                      = 1 iff {x : x is a man} <= {x : x smokes} here {x : x is a man} <= A and {x : x smokes} <= A
```
```
F('no man smokes') = | {x : man_f(x) = 1} & {x : smokes_f(x) = 1} | = 0
                   = 1 iff | {x : x is a man} & {x : x smokes} | = 0 here {x : x is a man} <= A and {x : x smokes} <= A
``` 
