...
- $⟦\text{looked at}⟧^M$ = $\lambda (x,y) \in A \times A$. 1 iff $(x, y) \in$ { $(x,y) \mid x\ \text{looked at}\ y$ } [ type: $(e \times e) \rightarrow t$ ]
...
- As a result, ... 
  - $⟦\text{looked at}⟧^M$ = $\lambda y \lambda x.\text{looked.at}'(x, y)$ [Type: $e \rightarrow e \rightarrow t$]
